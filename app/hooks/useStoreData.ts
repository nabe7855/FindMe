...
export const useStoreData = (initialCriteria: SearchCriteria | null) => {
  ...
  const filterStores = useCallback((stores: Store[], currentCriteria: SearchCriteria | null): Store[] => {
    ...
    return stores.filter(store => {
      const prefectureMatch = currentCriteria.prefecture === '全国' || store.prefecture === currentCriteria.prefecture;
      const genreMatch = currentCriteria.genre === 'すべて' || store.genre === currentCriteria.genre;
      const keywordMatch = !currentCriteria.keyword || 
                           store.name.toLowerCase().includes(currentCriteria.keyword.toLowerCase()) ||
                           store.catch_phrase.toLowerCase().includes(currentCriteria.keyword.toLowerCase()) ||
                           store.area.toLowerCase().includes(currentCriteria.keyword.toLowerCase());
