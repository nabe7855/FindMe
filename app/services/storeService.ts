 ...
 * 将来的に実際のデータベースやAPIとの通信を行う場合、このファイル内の実装を置き換えることで対応します。
...
export const getAllStores = (): Promise<Store[]> => {
  ...
  return new Promise(resolve => {
    setTimeout(() => {
      // 実際にはここで fetch('/api/stores') などを呼び出す
      resolve(mockStores);
 ...
 * @param {string} id - 取得したい店舗のID。
 * @returns {Promise<Store | undefined>} 見つかった店舗データ、または見つからない場合はundefinedを解決するPromise。
 */
export const getStoreById = (id: string): Promise<Store | undefined> => {
  console.log(`Fetching store with id: ${id}...`);
  return new Promise(resolve => {
    setTimeout(() => {
      // 実際にはここで fetch(`/api/stores/${id}`) などを呼び出す
      const store = mockStores.find(s => s.id === id);
      resolve(store);
    }, API_SIMULATION_DELAY / 2); // ID指定の場合は少し速く返すシミュレーション
  });
};
 ...
 */
export const getLatestReviewsWithStoreInfo = (count: number): Promise<ReviewWithStoreInfo[]> => {
    console.log(`Fetching ${count} latest reviews...`);
    return new Promise(resolve => {
        setTimeout(() => {
            const allReviews = mockStores.flatMap(store =>
